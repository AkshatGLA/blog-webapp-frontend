...
const UpdateBlog = () => {
    ...
    // FIX 1: Initialize with empty string fallback to prevent "Uncontrolled Input" error
    ...
    const updateBlogHandler = async () => {
        ...
        try {
            setLoading(true)
            // FIX 2: Use Dynamic URL from .env
            const url = `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_UPDATE_BLOG}${id}`;
    ...
    const togglePublishUnpublish = async (action) => {
        try {
            ...
            // FIX 3: Pass params in the 3rd argument (config), not the 2nd (body)
            const res = await axios.patch(url, {}, {
                params: {
                    publish: action // Backend expects 'publish', not 'action' based on your controller
                },
                withCredentials: true
            })
            if (res.data.success) {
    ...
    const deleteBlog = async () => {
        try {
            // FIX 2: Use Dynamic URL from .env
            const url = `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_DELETE_BLOG}${id}`;
            ...
            <div className='max-w-6xl mx-auto mt-8'>
                <Card className="w-full bg-white dark:bg-gray-800 p-5 space-y-2">
                    ...
                    <div>
                        ...
                        <Select onValueChange={selectCategory} className="dark:border-gray-300" defaultValue={blogData.category}>
